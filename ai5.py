...
def main():
    ...
    with col1:
        ...
        if uploaded_file and st.button("🎯 معالجة الملف المرفوع"):
            # Save uploaded file
            temp_file = tempfile.NamedTemporaryFile(delete=False, suffix=".wav")
        ...
        user_text = st.text_area("اكتب رسالتك هنا:", height=100, 
                                placeholder="مثال: ما هي مواعيد العيادة؟")
        
        if st.button("📤 إرسال الرسالة") and user_text.strip():
        ...
        for i, msg in enumerate(st.session_state.chat_history):
            ...
            if f"audio_{i}" in st.session_state:
                audio_file = st.session_state[f"audio_{i}"]
                if os.path.exists(audio_file):
